...
public class VictimBase : MonoBehaviour, ICanDead
{
    ...
    [Tooltip("残り時間を持つクラス")] protected CountDownTimer _countDownTimer = default;
    ...
    public float[] Timeline => _timeline;

    /// <summary> 死亡キャラのプレハブ </summary>
    public GameObject DeadCharaPrefab => _deadCharaPrefab;
    ...
    {
        for (var i = 0; i < _patrolNpcs.Length; i++)
        {
            if (i != num)
            {
